...
Процедура ВыполнитьТестОбъекта(Знач ТестируемыйОбъект, Знач Библиотека)
	...
	Библиотека.СоздатьОбъектИзСтруктуры(Представление);
...
Процедура ТестСКД(Команда)
	...
	Для Каждого ТекСКД Из АдресаФайлов Цикл
		...
		ВремФайл.Записать("d:\tmp\DCSTest\orig\" + ТекСкд.Ключ + ".json");
		
		ВремФайл = Новый ТекстовыйДокумент();
		ВремФайл.УстановитьТекст(ПолучитьИзВременногоХранилища(ТекСКД.Значение.Второй));
		ВремФайл.Записать("d:\tmp\DCSTest\test\" + ТекСкд.Ключ + ".json");
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура ТестСКДНаСервере(АдресаФайлов = Неопределено)
...
Функция ПреобразованиеДанных()
	...
	//Возврат ВнешниеОбработки.Создать("D:\repo\SerLib1C\dp\bin\ктв_ПреобразованиеДанных.epf");
