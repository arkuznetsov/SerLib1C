...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	#Если ВебКлиент Тогда
		ВызватьИсключение "Обработка не предназначена для запуска из WebКлиента";
	#КонецЕсли
...
Процедура Тест(Команда)
	...
	Для й = 0 По ТестируемыеОбъекты.ВГраница() Цикл
		...
		ОбъектыДляТестирования.Добавить(ТестируемыеОбъекты[й]);
		
		Если ОбъектыДляТестирования.Количество() < РазмерПорции
		   И й < ТестируемыеОбъекты.ВГраница() Тогда
			Продолжить;
		КонецЕсли;
		...
		ВыполнитьТестОбъектов(ОбъектыДляТестирования, АдресаФайлов, Ошибки);
	...
	ПоказатьСообщение("Ошибок: "               + Формат(Ошибки.Количество(), "ЧН=; ЧГ="));
	ПоказатьСообщение("Начало выполнения: "    + Формат(НачВремя, "ЧН=; ЧГ="));
	ПоказатьСообщение("Окончание выполнения: " + Формат(КонВремя, "ЧН=; ЧГ="));
	ПоказатьСообщение("Время выполнения: "     + Формат(КонВремя - НачВремя, "ЧН=; ЧГ=") + "мс.");
	ПоказатьСообщение("Скорость всего: "       + Формат(й / (КонВремя - НачВремя), "ЧН=; ЧГ=") + "об./мс.");
...
//	  *ПравилаВыгрузки          - Массив из Структура     - описание правил дополнения результата выгрузки объекта
		...
		Если ТекМетаОбъект.Иерархический
		   И ТекМетаОбъект.ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				СтруктураСсылки = СтруктураСсылки(Выборка.Ссылка, Новый Массив());
				СтруктураСсылки.ПравилаВыгрузки.Добавить(ПравилоВыгрузки);
...
&НаСервере
Функция СтруктураСсылки(Ссылка, ПравилаВыгрузки)

	Возврат Новый Структура("Ссылка, ПравилаВыгрузки", Ссылка, ПравилаВыгрузки);

КонецФункции // СтруктураСсылки()

// Процедура - обработчик команды "Тест" формы на сервере
//
&НаСервере
Процедура ВыполнитьТестОбъектов(Знач ОбъектыДляТестирования, АдресаФайлов = Неопределено, Ошибки = Неопределено)
	...
	Для Каждого ТекОписание Из ОбъектыДляТестирования Цикл
		...
		МетаПолноеИмя = ТекОписание.Ссылка.Метаданные().ПолноеИмя();
		УИД = СокрЛП(ТекОписание.Ссылка.УникальныйИдентификатор());

		ТекстСообщения = СтрШаблон("Тестирование объекта ""%1"" (%2: %3)...",
		                           ТекОписание.Ссылка,
		                           МетаПолноеИмя,
		                           УИД);
		...
		Исключение
			...
			ТекстОшибки = СтрШаблон("Ошибка сериализации объекта ""%1 (%2: %3)"": %4%5",
			                        ...
			                        Символы.ПС,
			                        ТекстОшибки);
			Ошибки.Вставить(СтрШаблон("%1.%2", МетаПолноеИмя, УИД), ТекстОшибки);
			Продолжить;
		КонецПопытки;
		...
		Если ВыгрузитьРезультатыТеста Тогда
			...
			АдресаФайлов.Вставить(СтрШаблон("%1.%2", МетаПолноеИмя, УИД), Хранилище);
		КонецЕсли;
...
Процедура ТестКоллекцийНаСервере(АдресаФайлов = Неопределено)
	...
	Для Каждого ТекСтрока Из ТестТаблица Цикл
		...
		Если Лев(ТекСтрока.Имя, 1) = "?" Тогда
			Продолжить;
		КонецЕсли;
		ТестСписок.Добавить(ТекСтрока.Ссылка, ТекСтрока.ПолноеИмя, Истина, ТекСтрока.Картинка);
...
//
&НаСервере
Процедура ТестСКДНаСервере(Знач ОтчетыДляТестирования, АдресаФайлов = Неопределено)
	...
	Для Каждого ТекИмяОтчета Из ОтчетыДляТестирования Цикл
		...
		Для Каждого ТекМакет Из ТекОтчет.Макеты Цикл
			...
			Сообщить(СтрШаблон("Преобразование объекта ""Отчет.%1.%2"" (%3)...",
			                   ТекОтчет.Имя,
			                   ТекМакет.Имя,
			...
			Попытка
				...
				Хранилище = Новый Структура("Первый",
				                            ПоместитьВоВременноеХранилище(ТекстОбъекта, ЭтотОбъект.УникальныйИдентификатор));
			...
			Исключение
				ТекстОшибки = СтрШаблон("Ошибка восстановления СКД: %1%2",
				                        Символы.ПС,
			...
			ТекстОбъекта = Библиотека.ЗаписатьОписаниеОбъектаВJSON(Представление);
			ПутьКМакету = СтрШаблон("%1.%2", ТекОтчет.Имя, ТекМакет.Имя);
			АдресаФайлов[ПутьКМакету].Вставить("Второй",
			                                   ПоместитьВоВременноеХранилище(ТекстОбъекта, ЭтотОбъект.УникальныйИдентификатор));
		КонецЦикла;
	КонецЦикла;
...
Функция ПолучитьНастройки(Знач ПутьКФайлуНастроек = "")
	...
	#Если НЕ ВебКлиент Тогда
		Попытка
			ДанныеНастроек = Новый ДвоичныеДанные(ПутьКФайлуНастроек);
...
//  ПутьКОбработке    - Строка    - путь к файлу внешней обработки
// 
// Возвращаемое значение:
...
//
// Параметры:
//  АдресОбработки    - Строка    - адрес хранилища внешней обработки
//  ИмяОбработки      - Строка    - имя внешней обработки
// 
// Возвращаемое значение:
