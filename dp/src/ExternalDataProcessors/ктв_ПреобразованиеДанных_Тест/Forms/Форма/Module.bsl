...
Процедура ТестСКДНаСервере()
	...
	Для Каждого ТекОтчет Из Метаданные.Отчеты Цикл
		Для Каждого ТекМакет Из ТекОтчет.Макеты Цикл
			...
			Если ТипЗнч(ТестируемыйОбъект) = Тип("СхемаКомпоновкиДанных") Тогда
				Продолжить;
			КонецЕсли;
			Сообщить(СтрШаблон("Преобразование объекта ""%1"" (%2)...", ТекОтчет.Имя, ТипЗнч(ТестируемыйОбъект)));
				
			Представление = Библиотека.СКДВСтруктуру(ТестируемыйОбъект);
			ПроверитьСовместимостьСJSON(Представление);
			Продолжить;
			ТекстОбъекта = Библиотека.ЗаписатьОписаниеОбъектаВJSON(Представление);
			Представление = Библиотека.ПрочитатьОписаниеОбъектаИзJSON(ТекстОбъекта);
			Библиотека.СКДИзСтруктуры(Представление);
		КонецЦикла;
	КонецЦикла;
