...
//
// TODO: Реализовать обработку всех возможных имен типов
Функция ПолучитьОписаниеВидаОбъектов(Имя, ТекИБ = Неопределено) Экспорт
	...
	Для Каждого ТекОписание Из ОписаниеТипов Цикл
		Для Каждого ТекИмя Из ТекОписание.Значение.ИменаКоллекции Цикл
			ИменаКоллекций.Вставить(ТекИмя, ТекОписание.Ключ);
		КонецЦикла;
	КонецЦикла;
	
	ДанныеМакета = Неопределено;
	
	Если НЕ ОписаниеТипов.Свойство(ИменаКоллекций[НИмя], ДанныеМакета) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	ОписаниеТипа.Вставить("Имя"            , ИменаКоллекций[НИмя]);
...
Процедура ЗаполнитьОбъектИзСтруктуры(Объект
	...
	Иначе
		...
		КонецЕсли;
		Для Каждого ТекЭлемент Из ВремСтруктура Цикл
			ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.Таб + ТекЭлемент.Ключ;
		КонецЦикла;
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция СоздатьОбъектИзСтруктуры(СтруктураОбъекта
	...
	МетаОбъект = Метаданные.НайтиПоПолномуИмени(СтрШаблон("%1.%2", ОписаниеВида.Имя, ОписаниеВида.ИмяТипа));
