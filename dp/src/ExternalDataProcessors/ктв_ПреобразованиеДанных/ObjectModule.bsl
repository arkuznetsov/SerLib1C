...
Функция ЗначениеИзСтруктуры(СтруктураЗначения,
	...
	Если ЭтоКоллекция(СтруктураЗначения) Тогда
		Возврат КоллекцияИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки, ПреобразовыватьПрикладныеТипы);
	КонецЕсли;
...
// 
// Параметры:
//   НовыйРежим    - Булево    - новое значение флага
//
Процедура УстановитьБезопасныйРежимПД(НовыйРежим) Экспорт
...
//   Произвольный     - результат преобразования
//
Функция КоллекцияИзСтруктуры(СтруктураЗначения,
                             ПолучатьБитыеСсылки = Ложь,
                             ПреобразовыватьПрикладныеТипы = Истина) Экспорт
...
Процедура ТЧОбъектаИзСтруктуры(Объект,
	...
	Для й = 0 По МассивСтрок.ВГраница() Цикл
		...
		Попытка
			СтрокаЗначенийИзСтруктуры(НоваяСтрока, ТекСтрока, ПолучатьБитыеСсылки, Реквизиты);
		Исключение
			ТекстОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
//   ПреобразовыватьПрикладныеТипы      - Булево         - Истина - прикладные типы
//                                                         (Объекты, Ссылки, НаборыЗаписей и т.п.)
//                                                         будут прочитаны из сериализованого представления
//                                                         Ложь - прикладные типы будут прочитаны в сериализованом виде
//
