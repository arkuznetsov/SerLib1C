...
Функция ОписаниеТиповВСтруктуру(ОписаниеТипов) Экспорт
	...
	Если ОписаниеТипов.КвалификаторыЧисла.ДопустимаяДлина = ДопустимыйЗнак.Неотрицательный Тогда
...
Функция СвойствоИерархическогоОбъектаИспользуется(Объект, МетаСвойство, МетаОбъект = Неопределено)
	...
	Если СтрокаНачиснаетсяС(МетаОбъект.ПолноеИмя(), "Справочник.")
	   И МетаОбъект.ВидИерархии = Метаданные.СвойстваОбъектов.ВидИерархии.ИерархияГруппИЭлементов
	 ИЛИ СтрокаНачиснаетсяС(МетаОбъект.ПолноеИмя(), "ПланВидовХарактеристик.") Тогда
...
Функция ОбработчикЧтенияЗначенийJSON(Свойство, Значение, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		...
		ИначеЕсли СтрокаНачиснаетсяС(Значение, "/UnknownType(") Тогда
			Если Прав(Значение, 2) = ")/" Тогда
				Значение = ЗначениеИзСтрокиВнутр(Сред(Значение, 14, СтрДлина(Значение) - 15));
			КонецЕсли;
		Иначе
			ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип значения JSON: %1", Значение));
		КонецЕсли;
	Иначе
		Возврат Значение;
	КонецЕсли;
...
Процедура ЗаполнитьОбъектИзСтруктуры(Объект
	...
	Иначе
		ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип объекта: %1", СтруктураОбъекта.Ссылка.Тип));
	КонецЕсли;
...
Функция СсылкаВСтруктуру(Значение) Экспорт
	...
	Если МетаданныеЗначения = Неопределено Тогда
		Возврат Значение;
	ИначеЕсли СтрокаНачиснаетсяС(МетаданныеЗначения.ПолноеИмя(), "Перечисление.") Тогда
		ИмяПредопределенныхДанных = "";
		Если ЗначениеЗаполнено(Значение) Тогда
		...
		ОписаниеЗначения.Вставить("Тип"                      , МетаданныеЗначения.ПолноеИмя());
		ОписаниеЗначения.Вставить("Ид"                       , ИмяПредопределенныхДанных);
		ОписаниеЗначения.Вставить("ИдПолный"                 , ЗначениеВСтрокуВнутр(Значение));
		ОписаниеЗначения.Вставить("Предопределенный"         , Истина);
	...
	Иначе
		...
		ОписаниеЗначения = Новый Структура();
		ОписаниеЗначения.Вставить("Тип"                      , МетаданныеЗначения.ПолноеИмя());
		ОписаниеЗначения.Вставить("Ид"                       , СокрЛП(Значение.УникальныйИдентификатор()));
		ОписаниеЗначения.Вставить("ИдПолный"                 , ЗначениеВСтрокуВнутр(Значение));
...
Функция РегистрПоОтборуВСтруктуру(Регистр, Отбор) Экспорт
	...
	Иначе
		ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип объекта: %1", Регистр));
	КонецЕсли;
...
Функция ДвиженияДокументаВСтруктуру(Объект, СписокРегистров = Неопределено, ИсключаяРегистры = Неопределено) Экспорт
	...
	Для Каждого ТекДвижения Из МетаОбъект.Движения Цикл
		...
		Иначе
			ПоказатьСообщение(СтрШаблонВнутр("Неизвестный тип объекта: %1", ТекДвижения.ПолноеИмя()));
		КонецЕсли;
...
Функция ТипДополненияПериодаКомпоновкиДанныхВСтроку(Значение)
	...
	Если СтрокаНачиснаетсяС(СИ.ВерсияПриложения, "8.3.12") Тогда
...
Функция СтрШаблонВнутр(Текст, Замена1 = Неопределено, Замена2 = Неопределено, Замена3 = Неопределено,
	...
	Исключение
		ПоказатьСообщение(СтрШаблонВнутр("Ошибка вызова метода СтрШаблон(): %1",
			                             ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())));
	КонецПопытки;
...
Функция СтрокаНачиснаетсяС(Строка, Подстрока = "", УчитыватьРегистр = Ложь)
...
КонецФункции // СтрокаНачиснаетсяС()
