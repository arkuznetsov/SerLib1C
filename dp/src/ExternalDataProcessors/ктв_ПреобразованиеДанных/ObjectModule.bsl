...
// Проверяет, что переданное значение является представлением ссылочного объекта
//
// Параметры:
//  Значение    - Структура - представление ссылочного объекта
// 
// Возвращаемое значение:
//  Булево      - Истина - передано ссылочное значение,
...
Функция ЭтоСсылка(Значение) Экспорт
	...
	Если НЕ Значение.Свойство("__Тип") Тогда
		Возврат Ложь;
	КонецЕсли;

	ОписаниеТипа = ПолучитьОписаниеВидаОбъектов(Значение.__Тип);
	...
	Возврат СсылочныеКоллекции.Получить(ОписаниеТипа.Имя);
...
Функция СсылкаСуществует(Значение) Экспорт
    ...
    ОписаниеТипа = ПолучитьОписаниеВидаОбъектов(ТипЗнч(Значение));
    ...
    Запрос.Текст = СтрЗаменить(Запрос.Текст,
                               "//[ИмяТаблицы]",
                               СтрШаблон("%1.%2", ОписаниеТипа.Имя, ОписаниеТипа.ИмяТипа));
...
Функция ДанныеФормыВСтруктуру(Форма
	...
	СтруктураДанных.Вставить("__Тип"    , "ДанныеФормы");
...
Процедура ДанныеФормыИзСтруктуры(Форма
	...
	Если НЕ СтруктураДанных.__Тип = "ДанныеФормы" Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ТекРеквизит Из СтруктураДанных Цикл
		...
		ЗначениеДляВосстановления = ЗначениеИзСтруктуры(ТекРеквизит.Значение);
		
		Если ТипРеквизита = Тип("ДанныеФормыСтруктура")
		 ИЛИ ТипРеквизита = Тип("ДанныеФормыСтруктураСКоллекцией")
		 ИЛИ ТипРеквизита = Тип("ДанныеФормыКоллекция")
		 ИЛИ ТипРеквизита = Тип("ДанныеФормыДерево") Тогда
			Форма.ЗначениеВРеквизитФормы(ЗначениеДляВосстановления, ТекРеквизит.Ключ);
		Иначе
			Форма[ТекРеквизит.Ключ] = ЗначениеДляВосстановления;
		КонецЕсли;
