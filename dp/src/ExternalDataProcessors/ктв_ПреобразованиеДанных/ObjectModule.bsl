...
Функция ПолучитьОписаниеВидаОбъектов(Имя, ТекИБ = Неопределено) Экспорт
	...
	ОписаниеТипов = ПрочитатьОписаниеОбъектаИзJSON(МакетОписанияТипов.ПолучитьТекст());

	ДанныеМакета = Неопределено;
	
	Если НЕ ОписаниеТипов.Свойство(КлючТипа, ДанныеМакета) Тогда
		Возврат Неопределено;
	КонецЕсли;

	ОписаниеТипа.Вставить("Коллекция"      , ?(ТекИБ = Неопределено,
											   Вычислить(ДанныеМакета.Коллекция),
											   ТекИБ.Вычислить(ДанныеМакета.Коллекция)));
	ОписаниеТипа.Вставить("МетаКоллекция"  , ?(ТекИБ = Неопределено,
											   Метаданные[ДанныеМакета.Коллекция],
											   ТекИБ.Метаданные[ДанныеМакета.Коллекция]));
	ОписаниеТипа.Вставить("Имя"            , КлючТипа);
	ОписаниеТипа.Вставить("ИмяКоллекции"   , ДанныеМакета.Коллекция);
	ОписаниеТипа.Вставить("Представление"  , ДанныеМакета.Представление);
	ОписаниеТипа.Вставить("ПредставлениеМ" , ДанныеМакета.ПредставлениеМ);
	ОписаниеТипа.Вставить("Конструктор"    , ДанныеМакета.Конструктор);
	ОписаниеТипа.Вставить("КонструкторАльт", ДанныеМакета.КонструкторАльт);
	ОписаниеТипа.Вставить("Картинка"       , Неопределено);
	ОписаниеТипа.Вставить("КартинкаОбъекта", Неопределено);
	#Если Клиент Тогда
	ОписаниеТипа.Картинка        = БиблиотекаКартинок[ДанныеМакета.Картинка];
	ОписаниеТипа.КартинкаОбъекта = БиблиотекаКартинок[ДанныеМакета.КартинкаОбъекта];
	#КонецЕсли
...
Функция ОписаниеТиповВСтруктуру(ОписаниеТипов) Экспорт
	...
	СтруктураОписаниеТипов = Новый Структура("__Тип,
                                             |Типы,
											 |КвалификаторыЧисла,
											 |КвалификаторыСтроки,
											 |КвалификаторыДаты,
...
Функция ОбработчикЧтенияЗначенийJSON(Свойство, Значение, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		...
		ИначеЕсли Лев(Значение, 13) = "/UnknownType(" Тогда
			Если Прав(Значение, 2) = ")/" Тогда
...
#КонецОбласти // ЗаписьОбъектоВJSON
...
Функция МассивИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь)
	...
	Для Каждого ТекЗначение Из СтруктураЗначения Цикл
		ВремМассив.Добавить(ЗначениеИзСтруктуры(ТекЗначение, ПолучатьБитыеСсылки));
	КонецЦикла;
...
Процедура СтрокаЗначенийИзСтруктуры(СтрокаЗначений
	...
	Для Каждого ТекСвойство Из СписокСвойств Цикл
		СтрокаЗначений[ТекСвойство] = ЗначениеИзСтруктуры(СтруктураЗначений[ТекСвойство], ПолучатьБитыеСсылки);
	КонецЦикла;
...
Функция ТаблицаЗначенийИзСтруктуры(СтруктураЗначения
	...
	Для Каждого ТекСтрока Из СтруктураЗначения Цикл
		...
		СтрокаЗначенийИзСтруктуры(НоваяСтрока, ТекСтрока, ПолучатьБитыеСсылки, МассивКолонок);
	КонецЦикла;
...
Функция ДеревоЗначенийИзСтруктуры(СтруктураЗначения
	...
	СтрокиДереваИзСтруктуры(ВремДерево.Строки, СтруктураЗначения.Строки, ПолучатьБитыеСсылки, МассивКолонок);
...
Функция СсылкаИзСтруктуры(СтруктураЗначения, ПолучатьБитыеСсылки = Ложь) Экспорт
	...
	Если НЕ Найти(СтруктураЗначения.ИдПолный, "00000000000000000000000000000000") = 0 Тогда
		Возврат Результат;
	КонецЕсли;
...
Функция ДвиженияДокументаВСтруктуру(Объект, СписокРегистров = Неопределено, ИсключаяРегистры = Неопределено) Экспорт
	...
	Для Каждого ТекДвижения Из МетаОбъект.Движения Цикл
		...
		Если ВРег(ОписаниеВида.Имя) = ВРег("РегистрСведений") Тогда
			СтруктураОбъекта = НаборЗаписейРегистраСведенийВСтруктуру(Объект.Движения[ТекДвижения.Имя]);
		ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрНакопления") Тогда
			СтруктураОбъекта = НаборЗаписейРегистраНакопленияВСтруктуру(Объект.Движения[ТекДвижения.Имя]);
		ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрБухгалтерии") Тогда
			СтруктураОбъекта = НаборЗаписейРегистраБухгалтерииВСтруктуру(Объект.Движения[ТекДвижения.Имя]);
		ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрРасчета") Тогда
			СтруктураОбъекта = НаборЗаписейРегистраРасчетаВСтруктуру(Объект.Движения[ТекДвижения.Имя]);
		КонецЕсли;
...
Процедура ДвиженияДокументаИзСтруктуры(Объект
	...
	Для Каждого ТекДвижения Из МетаОбъект.Движения Цикл
		...
		Если ВРег(ОписаниеВида.Имя) = ВРег("РегистрСведений") Тогда
			НаборЗаписейРегистраСведенийИзСтруктуры(Объект.Движения[ТекДвижения.Имя],
			                                        СтруктураДвижений[ТекДвижения.Имя],
			                                        ПолучатьБитыеСсылки);
		ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрНакопления") Тогда
			НаборЗаписейРегистраНакопленияИзСтруктуры(Объект.Движения[ТекДвижения.Имя],
			                                          СтруктураДвижений[ТекДвижения.Имя],
			                                          ПолучатьБитыеСсылки);
		ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрБухгалтерии") Тогда
			НаборЗаписейРегистраБухгалтерииИзСтруктуры(Объект.Движения[ТекДвижения.Имя],
			                                           СтруктураДвижений[ТекДвижения.Имя],
			                                           ПолучатьБитыеСсылки);
		ИначеЕсли ВРег(ОписаниеВида.Имя) = ВРег("РегистрРасчета") Тогда
			НаборЗаписейРегистраРасчетаИзСтруктуры(Объект.Движения[ТекДвижения.Имя],
			                                       СтруктураДвижений[ТекДвижения.Имя],
			                                       ПолучатьБитыеСсылки);
		КонецЕсли;
...
Процедура СправочникИзСтруктуры(Объект
	...
	Если МетаОбъект.Иерархический Тогда
		СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "Родитель", ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
	КонецЕсли;
			
	Если МетаОбъект.Владельцы.Количество() > 0 Тогда
		СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "Владелец", ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
	КонецЕсли;
	...
	Для Каждого ТекРеквизит Из МетаОбъект.Реквизиты Цикл
		Если МетаОбъект.Иерархический И МетаОбъект.ВидИерархии = МетаСвойства.ВидИерархии.ИерархияГруппИЭлементов Тогда
			...
			КонецЕсли;
		КонецЕсли;
		СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, ТекРеквизит.Имя, ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
	КонецЦикла;
...
Процедура ПланСчетовИзСтруктуры(Объект
	...
	Для Каждого ТекПризнак Из МетаОбъект.ПризнакиУчета Цикл
		СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, ТекПризнак.Имя, ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
	КонецЦикла;
...
Процедура ДокументИзСтруктуры(Объект
	...
	Если ИспользоватьСвойство("Движения", СписокСвойств, ИсключаяСвойства) И СтруктураОбъекта.Свойство("Движения") Тогда
		ДвиженияДокументаИзСтруктуры(Объект, СтруктураОбъекта.Движения, ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
	КонецЕсли;
...
Процедура БизнесПроцессИзСтруктуры(Объект
	...
	СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "ВедущаяЗадача", ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
...
Процедура ЗадачаИзСтруктуры(Объект
	...
	СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "БизнесПроцесс", ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
	СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "Выполнена", , СписокСвойств, ИсключаяСвойства);
	СвойствоОбъектаИзСтруктуры(Объект, СтруктураОбъекта, "ТочкаМаршрута", ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
...
Процедура НаборЗаписейРегистраСведенийИзСтруктуры(НаборЗаписей
	...
	Если МетаОбъект.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.ПодчинениеРегистратору Тогда
		НаборЗаписей.Отбор.Регистратор.Установить(ЗначениеИзСтруктуры(СтруктураОбъекта.Отбор.Регистратор, ПолучатьБитыеСсылки));
	Иначе
		Если НЕ МетаОбъект.ПериодичностьРегистраСведений = Метаданные.СвойстваОбъектов.ПериодичностьРегистраСведений.Непериодический Тогда
			НаборЗаписей.Отбор.Период.Установить(ЗначениеИзСтруктуры(СтруктураОбъекта.Отбор.Период, ПолучатьБитыеСсылки));
		КонецЕсли;
		...
		Для Каждого ТекПоле Из МетаОбъект.Измерения Цикл
			Если НЕ (СтруктураОбъекта.Отбор.Свойство(ТекПоле.Имя) И ИспользоватьСвойство(ТекПоле.Имя, СписокСвойств, ИсключаяСвойства)) Тогда
				Продолжить;
			КонецЕсли;
			НаборЗаписей.Отбор[ТекПоле.Имя].Установить(ЗначениеИзСтруктуры(СтруктураОбъекта.Отбор[ТекПоле.Имя], ПолучатьБитыеСсылки));
		КонецЦикла;
	КонецЕсли;
...
Процедура НаборЗаписейРегистраБухгалтерииИзСтруктуры(НаборЗаписей
	...
	Для Каждого ТекЗапись Из СтруктураОбъекта.Записи Цикл
		...
		Если МетаОбъект.Корреспонденция Тогда
			...
			ЗначенияСубконтоЗаписиИзСтруктуры(НоваяЗапись, ТекЗапись, "Дт", ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
			ЗначенияСубконтоЗаписиИзСтруктуры(НоваяЗапись, ТекЗапись, "Кт", ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
		Иначе
			...
			ЗначенияСубконтоЗаписиИзСтруктуры(НоваяЗапись, ТекЗапись, "", ПолучатьБитыеСсылки, СписокСвойств, ИсключаяСвойства);
		КонецЕсли;
...
Функция ТипДополненияПериодаКомпоновкиДанныхВСтроку(Значение)
	...
	Если Лев(СИ.ВерсияПриложения, 6) = "8.3.12" Тогда
