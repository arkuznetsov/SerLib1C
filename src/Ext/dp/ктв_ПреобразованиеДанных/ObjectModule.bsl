...
Функция ЗначениеИзСтруктуры(СтруктураЗначения) Экспорт
	...
	Если ТипЗнч(СтруктураЗначения) = Тип("Строка")
	 ...
	 ИЛИ ТипЗнч(СтруктураЗначения) = Тип("Дата") Тогда
...
Функция НаборЗаписейРегистраНакопленияВСтруктуру(НаборЗаписей, СписокСвойств = Неопределено, ИсключаяСвойства = Неопределено) Экспорт
	...
	Для Каждого ТекСтрока Из НаборЗаписей Цикл
		...
		СтруктураОбъекта.Вставить("Регистратор", ЗначениеВСтруктуру(ТекСтрока.Регистратор));
		
		Если ИспользоватьСвойство("НомерСтроки", СписокСвойств, ИсключаяСвойства) Тогда
			СтруктураОбъекта.Вставить("НомерСтроки", ТекСтрока.НомерСтроки);
		КонецЕсли;
