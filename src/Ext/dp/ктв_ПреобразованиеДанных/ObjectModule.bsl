...
Процедура ДобавитьПравилоВыгрузкиТипа(ТипИсточника, ФункцияДополнения, Модуль = Неопределено) Экспорт
	...
	ПравилаВыгрузкиТипов.Вставить(ТипИсточника, Новый Структура("ФункцияДополнения, Модуль", ФункцияДополнения));
...
Функция СсылкаИзСтруктуры(ОписаниеЗначения, ПолучатьБитыеСсылки = Ложь) Экспорт
	...
	Исключение
		ТекстОшибки = "Ошибка поиска ссылки для значения: " + Символы.ПС + ПодробноеПредставлениеОшибки();
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
